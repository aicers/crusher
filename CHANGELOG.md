...
- Fixed the issue where the node status was not updated in the manager server
  because Crusher disconnected after handling a single request. The manager
  server sends two consecutive requests (resource and ping) to check the node
  status. If Crusher disconnects after processing the first request, the
  subsequent request (ping, in this case) is not handled properly.
