...
pub(super) const SERVER_RETRY_INTERVAL: u64 = 3;

#[allow(clippy::struct_field_names)]
pub(super) struct Certs {
    pub(super) certs: Vec<CertificateDer<'static>>,
    pub(super) key: PrivateKeyDer<'static>,
    pub(super) ca_certs: RootCertStore,
}

impl Certs {
    pub(super) fn try_new(cert_pem: &[u8], key_pem: &[u8], ca_certs_pem: &[&[u8]]) -> Result<Self> {
    ...
    pub(super) fn to_cert_chain(raw: &[u8]) -> Result<Vec<CertificateDer<'static>>> {
    ...
    pub(super) fn to_private_key(raw: &[u8]) -> Result<PrivateKeyDer<'static>> {
    ...
    pub(super) fn to_ca_certs(ca_certs_pem: &[&[u8]]) -> Result<rustls::RootCertStore> {
...
pub(super) fn config(certs: &Certs) -> Result<Endpoint> {
