...
impl Client {
    ...
    pub async fn run(&mut self, shutdown: Arc<Notify>) -> Result<()> {
        ...
        tokio::select! {
            ...
            () = shutdown.notified() => {
                info!("Shutting down Manager server");
            }
        };
        Ok(())
    }
    ...
    /// Enters the minimal mode when a recoverable error occurs.
    ///
    /// If `health_check` is `true`, it just checks the connection and returns.
    pub async fn enter_minimal_mode(&mut self, health_check: bool) {
        println!("Entering minimal mode");
...
impl review_protocol::request::Handler for Client {
    ...
    async fn update_config(&mut self) -> Result<(), String> {
        info!("Manager server updated a new configuration");
