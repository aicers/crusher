...
async fn main() -> Result<()> {
    ...
    loop {
        ...
        .await
        {
            ...
            let health_check = e.downcast_ref::<std::io::Error>().map_or(false, |e| {
                matches!(
                    e.kind(),
                    ErrorKind::ConnectionAborted | ErrorKind::ConnectionReset | ErrorKind::TimedOut
                )
            });
            request_client.enter_minimal_mode(health_check).await;
        }
    }
}
...
async fn run(
    ...
    if guards.is_empty() {
        // TODO(jake): `last_timestamp_data` will be outdated in the remote mode.
        // https://github.com/aicers/crusher/issues/113#issuecomment-2544413384
        read_last_timestamp(&settings.last_timestamp_data).await?;
        // `guards` should be extended last. Otherwise, retry logic for reading `last_timestamp`
